Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/4153ddf403efe9440bdbce97bf2f4349"
{
	Properties
	{
 ...
 #define PROP_CUBEMAP 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					...
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.15), float(0.1) * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.1) * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.1) * blurMap.r, float(0.08));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.1) * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.15), float(0.1) * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.1) * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.1) * blurMap.r, float(0.08));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.6382833,0.5382356,0.6920712,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.6382833,0.5382356,0.6920712,1).rgb;
				}
				if (float4(0.4200333,0.3931231,0.5870164,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.4200333,0.3931231,0.5870164,1).rgb;
					lns.y = float4(0.4200333,0.3931231,0.5870164,1).a - lns.y * float4(0.4200333,0.3931231,0.5870164,1).a;
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.847), float(0), float(1), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				cubeMap.rgb *= poiThemeColor(poiMods, float4(1,1,1,1), float(0));
				#else
				float4 cubeMap = float4(poiThemeColor(poiMods, float4(1,1,1,1), float(0)), 1);
				#endif
				cubeMap.rgb *= float(1);
				#if defined(PROP_CUBEMAPMASK) || !defined(OPTIMIZER_ENABLED)
				float CubeMapMask = POI2D_SAMPLER_PAN(_CubeMapMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				float CubeMapMask = 1;
				#endif
				if (float(0))
				...
				if (float(0))
				{
					cubeMap.rgb = hueShift(cubeMap.rgb, float(0) + _Time.x * float(0));
				}
				CubeMapMask = min(CubeMapMask, lerp(1, poiLight.rampedLightMap, float(0)));
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, cubeMap.rgb, float(0) * CubeMapMask * cubeMap.a);
				poiFragData.baseColor.rgb *= lerp(1, cubeMap.rgb, float(0) * CubeMapMask * cubeMap.a);
				poiFragData.baseColor.rgb += cubeMap.rgb * float(0.682) * CubeMapMask * cubeMap.a;
				poiFragData.emission += cubeMap.rgb * float(0) * CubeMapMask * cubeMap.a;
			}
			#endif
			//ifex float(1)==0
			#ifdef POI_STYLIZED_StylizedSpecular
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(1))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.314)))) * float(1);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.314), 5)), float(0)) * float(1);
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(1))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.314)))) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.314), 5)), float(0)) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.2319224,0.2319224,0.2319224,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.2319224,0.2319224,0.2319224,1).a;
