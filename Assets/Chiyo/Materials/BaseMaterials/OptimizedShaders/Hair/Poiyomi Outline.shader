Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/16a7df375ea8b684ebee6d1b7668ec16"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMASK 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.501);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.501);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.501) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.095) * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.091), float(0.156) * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.1) * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.095) * blurMap.r, float(0.08));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.095) * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.091), float(0.156) * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.1) * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.095) * blurMap.r, float(0.08));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.4264112,0.5581353,0.6070304,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.4264112,0.5581353,0.6070304,1).rgb;
				}
				if (float4(0.2369172,0.3974111,0.4588114,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.2369172,0.3974111,0.4588114,1).rgb;
					lns.y = float4(0.2369172,0.3974111,0.4588114,1).a - lns.y * float4(0.2369172,0.3974111,0.4588114,1).a;
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(0,0.8549927,0.4953141,1).rgb);
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.501));
				float3 finalWrap = directColor + indirectColor;
				if (float(1))
				{
					finalWrap = clamp(finalWrap, _LightingMinLightBrightness, float(1));
				}
				else
				{
					finalWrap = max(finalWrap, _LightingMinLightBrightness);
				}
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.501));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.501) * poiLight.shadowMask);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(1))
				{
					case 0:
					{
						...
						matcapUV = noSknewViewNormal.rg * float(0.287) + 0.5;
						break;
					}
					case 1:
					{
						...
						matcapUV = float2(dot(worldViewRight, normal0), dot(worldViewUp, normal0)) * float(0.287) + 0.5;
						break;
					}
					case 2:
					{
				...
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(poiThemeColor(poiMods, float4(0.2411783,0.2411783,0.2411783,1).rgb, float(0)), float4(0.2411783,0.2411783,0.2411783,1).a);
				#else
				matcap = float4(poiThemeColor(poiMods, float4(0.2411783,0.2411783,0.2411783,1).rgb, float(0)), float4(0.2411783,0.2411783,0.2411783,1).a);
				#endif
				matcap.rgb *= float(5);
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(0), float(1), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,1,0.8652289,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,1,0.8652289,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,1,0.8652289,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(0.41));
				if (float(0) && float(0))
				{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.1669015,0.296527,0.418999,1).rgb, float(0)), float4(0.1669015,0.296527,0.418999,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.1669015,0.296527,0.418999,1).rgb, float(0)), float4(0.1669015,0.296527,0.418999,1).a);
				#endif
				...
				rim *= float4(0.1669015,0.296527,0.418999,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.1669015,0.296527,0.418999,1).rgb, float(0)));
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.401)))) * float(0.304);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.401), 5)), float(0.151)) * float(0.304);
				...
				if (float(1) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.0620674,0.07846346,0.1579265,1), float(0)) * lerp(1, poiLight.directColor, float(1)) * specMask * poiLight.occlusion * attenuation * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.0620674,0.07846346,0.1579265,1), float(0)) * lerp(1, poiLight.directColor, float(1)), saturate(specMask * poiLight.occlusion * attenuation * float(1)));
				}
				float3 vSpecMask = 0;
				#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.401)))) * float(0.304) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.401), 5)), float(0.151)) * float(0.304) * poiLight.vAttenuation * poiLight.vColor[index];
				...
				if (float(1) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.0620674,0.07846346,0.1579265,1), float(0)) * vSpecMask * poiLight.occlusion * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.0620674,0.07846346,0.1579265,1), float(0)), saturate(vSpecMask * poiLight.occlusion * float(1)));
				}
				#endif
			}
			#endif
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				if (float(3) == 0) // Poi Custom Light Color
				{
				...
				if (float(3) == 1) // More standard approach to light color
				{
				...
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), float(0.5));
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), float(0.5));
				if (float(1))
				{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -float(0.29) * hueShiftAlpha.b);
				poiFragData.baseColor = saturate(poiFragData.baseColor + float(0) * hueShiftAlpha.g);
				#endif
 ...
 #define PROP_EMISSIONMASK 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.2957615,0.3787588,0.5152035,1).rgb, float(0)), float4(0.2957615,0.3787588,0.5152035,1).a);
				poiFragData.baseColor = lerp(col, col * poiFragData.baseColor, float(1));
				if (float(1) == 2)
				{
