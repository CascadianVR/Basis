Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/86a3987a04bef764e9e892c3d4fc6d79"
{
	Properties
	{
 ...
 #define PROP_MATCAP2 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.112) * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0), float(0.637) * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0), float(0) * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.112) * blurMap.r, float(0.195));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.112) * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0), float(0.637) * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0), float(0) * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.112) * blurMap.r, float(0.195));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.7799658,0.7799658,0.7799658,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.7799658,0.7799658,0.7799658,1).rgb;
				}
				if (float4(0.7986599,0.7240834,0.7240834,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.7986599,0.7240834,0.7240834,1).rgb;
					lns.y = float4(0.7986599,0.7240834,0.7240834,1).a - lns.y * float4(0.7986599,0.7240834,0.7240834,1).a;
				...
				if (float4(1,1,1,0).a > 0)
				{
					...
					#endif
					shadow3rdColorTex.rgb = lerp(float3(1, 1, 1), shadow3rdColorTex.rgb, shadow3rdColorTex.a) * float4(1,1,1,0).rgb;
					lns.z = float4(1,1,1,0).a - lns.z * float4(1,1,1,0).a;
					indirectColor = lerp(indirectColor, shadow3rdColorTex.rgb, lns.z);
				}
				poiLight.rampedLightMap = lns.x;
				indirectColor = lerp(indirectColor, 1, lns.w * float4(0.1691187,0.6704556,1,1).rgb);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.111), float(0), float(0), float(0), float(0), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float3 finalRimColor = rimColor.rgb * lerp(1, poiFragData.baseColor, float(0.99));
				finalRimColor *= float(1.11);
				switch(float(0))
				{
			...
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min(float(0), float(0.45)), float(0.45), poiCam.vDotN));
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.095)))) * float(0.071);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0.48)))) * float(0.038);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.095), 5)), float(0.374)) * float(0.071);
				...
				float attenuation = min(poiLight.nDotLSaturated, lerp(poiLight.attenuation, 1, float(1)));
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(1,1,1,1), float(0)) * lerp(1, poiLight.directColor, float(0)) * specMask * poiLight.occlusion * attenuation * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(1,1,1,1), float(0)) * lerp(1, poiLight.directColor, float(0)), saturate(specMask * poiLight.occlusion * attenuation * float(1)));
				}
				float3 vSpecMask = 0;
				#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.095)))) * float(0.071) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0.48)))) * float(0.038) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.095), 5)), float(0.374)) * float(0.071) * poiLight.vAttenuation * poiLight.vColor[index];
