Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/9a625099afa549e41b91cfd648d6ee59"
{
	Properties
	{
 ...
 #define PROP_SET_HIGHCOLORMASK 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(1,0.00877352,0,1).rgb);
			...
			#endif
			//ifex float(0)==0 && float(1)==0 && float(1)==0 && float(0)==0
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.2)))) * float(0.262);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.2), 5)), float(0.172)) * float(0.262);
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.2)))) * float(0.262) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.2), 5)), float(0.172)) * float(0.262) * poiLight.vAttenuation * poiLight.vColor[index];
