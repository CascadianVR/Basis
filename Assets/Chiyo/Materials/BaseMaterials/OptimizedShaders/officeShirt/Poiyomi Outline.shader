Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/7b54016f27b1f8841bb20ac656ee79d3"
{
	Properties
	{
 ...
 #define _RIMSTYLE_POIYOMI 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.509);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.509);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.509) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.325) * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.325) * blurMap.r, float(0.222));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.325) * blurMap.r);
					...
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.325) * blurMap.r, float(0.222));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.2228772,0.08691046,0.08077768,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.2228772,0.08691046,0.08077768,1).rgb;
				}
				if (float4(0.2232279,0.08650045,0.08021981,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.2232279,0.08650045,0.08021981,1).rgb;
					lns.y = float4(0.2232279,0.08650045,0.08021981,1).a - lns.y * float4(0.2232279,0.08650045,0.08021981,1).a;
					indirectColor = lerp(indirectColor, shadow2ndColorTex.rgb, lns.y);
				}
				if (float4(0.2232279,0.08650045,0.08021981,0).a > 0)
				{
					#if defined(PROP_SHADOW3RDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow3rdColorTex.rgb = lerp(float3(1, 1, 1), shadow3rdColorTex.rgb, shadow3rdColorTex.a) * float4(0.2232279,0.08650045,0.08021981,0).rgb;
					lns.z = float4(0.2232279,0.08650045,0.08021981,0).a - lns.z * float4(0.2232279,0.08650045,0.08021981,0).a;
					indirectColor = lerp(indirectColor, shadow3rdColorTex.rgb, lns.z);
				}
				poiLight.rampedLightMap = lns.x;
				indirectColor = lerp(indirectColor, 1, lns.w * float4(0.6396053,0.1374736,0.148159,1).rgb);
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.509));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.509));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.509) * poiLight.shadowMask);
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(1.06));
				if (float(0.482) && float(1))
				{
					viewDotNormal += lerp(0, (1 - poiLight.nDotLNormalized) * 3, float(0.482));
				}
				float rimStrength = float(0);
				float rimWidth = lerp( - .05, 1, float(0.737));
				...
				float rim = 1 - smoothstep(min(float(0.337), rimWidth), rimWidth, viewDotNormal);
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.391)))) * float(0.099);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.391), 5)), float(0.321)) * float(0.099);
				...
				if (float(1) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.1114872,0.03190586,0.03190586,1), float(0)) * lerp(1, poiLight.directColor, float(1)) * specMask * poiLight.occlusion * attenuation * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.1114872,0.03190586,0.03190586,1), float(0)) * lerp(1, poiLight.directColor, float(1)), saturate(specMask * poiLight.occlusion * attenuation * float(1)));
				}
				float3 vSpecMask = 0;
				#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.391)))) * float(0.099) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.391), 5)), float(0.321)) * float(0.099) * poiLight.vAttenuation * poiLight.vColor[index];
				...
				if (float(1) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.1114872,0.03190586,0.03190586,1), float(0)) * vSpecMask * poiLight.occlusion * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.1114872,0.03190586,0.03190586,1), float(0)), saturate(vSpecMask * poiLight.occlusion * float(1)));
				}
				#endif
			}
			#endif
