Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/0c01045156501294bbf8e66a70156fd7"
{
	Properties
	{
 ...
 #define PROP_MATCAPMASK 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					...
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.113) * blurMap.r, float(0.109));
				}
				else
				{
					...
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.113) * blurMap.r, float(0.109));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.7986599,0.6613454,0.6953436,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.7986599,0.6613454,0.6953436,1).rgb;
				}
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(1,0,0.00241253,1).rgb);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(1))
				{
					case 0:
					{
						...
						matcapUV = noSknewViewNormal.rg * float(0.433) + 0.5;
						break;
					}
					case 1:
					{
						...
						matcapUV = float2(dot(worldViewRight, normal0), dot(worldViewUp, normal0)) * float(0.433) + 0.5;
						break;
					}
					case 2:
					{
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(1);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0), 5)), float(0.562)) * float(1);
				...
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.07243511,0.09755541,0.1507121,1), float(0)) * lerp(1, poiLight.directColor, float(1)) * specMask * poiLight.occlusion * attenuation * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.07243511,0.09755541,0.1507121,1), float(0)) * lerp(1, poiLight.directColor, float(1)), saturate(specMask * poiLight.occlusion * attenuation * float(1)));
				}
				float3 vSpecMask = 0;
				#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0)))) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0), 5)), float(0.562)) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
				...
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.07243511,0.09755541,0.1507121,1), float(0)) * vSpecMask * poiLight.occlusion * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.07243511,0.09755541,0.1507121,1), float(0)), saturate(vSpecMask * poiLight.occlusion * float(1)));
				}
				#endif
			}
			#endif
