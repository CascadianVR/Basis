Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/205590f7c9436aa49a6b0dffd4baa134"
{
	Properties
	{
 ...
 #define PROP_MATCAP2 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.246) * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(1), float(0) * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0), float(0) * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.246) * blurMap.r, float(0.294));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.246) * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(1), float(0) * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0), float(0) * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.246) * blurMap.r, float(0.294));
				}
				lns = saturate(lns);
				...
				if (float4(0.6163016,0.7257689,0.7615293,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.6163016,0.7257689,0.7615293,1).rgb;
					lns.y = float4(0.6163016,0.7257689,0.7615293,1).a - lns.y * float4(0.6163016,0.7257689,0.7615293,1).a;
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(0.3130586,0.5736883,0.7077566,1).rgb);
