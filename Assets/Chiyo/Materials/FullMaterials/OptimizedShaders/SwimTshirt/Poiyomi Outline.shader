Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/69f86545b21518b499fab1ca7b946971"
{
	Properties
	{
 ...
 #define PROP_MATCAPMASK 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.401) * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.401) * blurMap.r, float(0.222));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.401) * blurMap.r);
					...
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.401) * blurMap.r, float(0.222));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.08154969,0.2658993,0.3812781,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.08154969,0.2658993,0.3812781,1).rgb;
				}
				if (float4(0.0822827,0.2663556,0.3813261,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.0822827,0.2663556,0.3813261,1).rgb;
					lns.y = float4(0.0822827,0.2663556,0.3813261,1).a - lns.y * float4(0.0822827,0.2663556,0.3813261,1).a;
					indirectColor = lerp(indirectColor, shadow2ndColorTex.rgb, lns.y);
				}
				if (float4(0.0822827,0.2663556,0.3813261,0).a > 0)
				{
					#if defined(PROP_SHADOW3RDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow3rdColorTex.rgb = lerp(float3(1, 1, 1), shadow3rdColorTex.rgb, shadow3rdColorTex.a) * float4(0.0822827,0.2663556,0.3813261,0).rgb;
					lns.z = float4(0.0822827,0.2663556,0.3813261,0).a - lns.z * float4(0.0822827,0.2663556,0.3813261,0).a;
					indirectColor = lerp(indirectColor, shadow3rdColorTex.rgb, lns.z);
				}
				poiLight.rampedLightMap = lns.x;
				indirectColor = lerp(indirectColor, 1, lns.w * float4(0.2250764,0.4595731,0.7254258,1).rgb);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0.166), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rimWidth = lerp( - .05, 1, float(0.525));
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.7892805,0.8778981,1,1).rgb, float(0)), float4(0.7892805,0.8778981,1,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.7892805,0.8778981,1,1).rgb, float(0)), float4(0.7892805,0.8778981,1,1).a);
				#endif
				...
				rim *= float4(0.7892805,0.8778981,1,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.7892805,0.8778981,1,1).rgb, float(0)));
 ...
 #define PROP_MATCAPMASK 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.5299,0.5299,0.5299,1).rgb, float(0)), float4(0.5299,0.5299,0.5299,1).a);
