Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/428a3ead0bb9ee64582544b13a4c4995"
{
	Properties
	{
 ...
 #define PROP_MATCAP 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.146) * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.146) * blurMap.r, float(0.189));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.146) * blurMap.r);
					...
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.146) * blurMap.r, float(0.189));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.7077566,0.7077566,0.7077566,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.7077566,0.7077566,0.7077566,1).rgb;
				}
				if (float4(0.5448383,0.5448383,0.5448383,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.5448383,0.5448383,0.5448383,1).rgb;
					lns.y = float4(0.5448383,0.5448383,0.5448383,1).a - lns.y * float4(0.5448383,0.5448383,0.5448383,1).a;
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(1,0,0.7618509,1).rgb);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(poiThemeColor(poiMods, float4(0.5561062,0.3398705,1,1).rgb, float(0)), float4(0.5561062,0.3398705,1,1).a);
				#else
				matcap = float4(poiThemeColor(poiMods, float4(0.5561062,0.3398705,1,1).rgb, float(0)), float4(0.5561062,0.3398705,1,1).a);
				#endif
				matcap.rgb *= float(1);
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
				...
				blendMatcap(poiLight, poiFragData, float(0.116), float(0), float(0), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(1.69));
				if (float(1) && float(1))
				{
					viewDotNormal += lerp(0, (1 - poiLight.nDotLNormalized) * 3, float(1));
				}
				float rimStrength = float(0);
				float rimWidth = lerp( - .05, 1, float(0.563));
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor = saturate(poiFragData.baseColor + float(0.01) * hueShiftAlpha.g);
				#endif
 ...
 #define PROP_MATCAP 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.3943972,0.04269191,0.5152035,1).rgb, float(0)), float4(0.3943972,0.04269191,0.5152035,1).a);
