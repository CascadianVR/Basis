Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/d2dc4bb3e46b0c24f915aa21b550da16"
{
	Properties
	{
 ...
 #define PROP_SET_HIGHCOLORMASK 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.672);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.672);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.672) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.113) * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.124), float(0.181) * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.155) * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.113) * blurMap.r, float(0.301));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.113) * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.124), float(0.181) * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.25), float(0.155) * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.113) * blurMap.r, float(0.301));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.6396053,0.6396053,0.6396053,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.6396053,0.6396053,0.6396053,1).rgb;
				}
				if (float4(0.5062425,0.5824313,0.6396053,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.5062425,0.5824313,0.6396053,1).rgb;
					lns.y = float4(0.5062425,0.5824313,0.6396053,1).a - lns.y * float4(0.5062425,0.5824313,0.6396053,1).a;
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(0.673178,0.5069463,0.449422,1).rgb);
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.672));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.672));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.672) * poiLight.shadowMask);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(1))
				{
					case 0:
					{
						...
						matcapUV = noSknewViewNormal.rg * float(0.142) + 0.5;
						break;
					}
					case 1:
					{
						...
						matcapUV = float2(dot(worldViewRight, normal0), dot(worldViewUp, normal0)) * float(0.142) + 0.5;
						break;
					}
					case 2:
					{
				...
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(poiThemeColor(poiMods, float4(0.9368213,0.6067858,0.4629994,1).rgb, float(0)), float4(0.9368213,0.6067858,0.4629994,1).a);
				#else
				matcap = float4(poiThemeColor(poiMods, float4(0.9368213,0.6067858,0.4629994,1).rgb, float(0)), float4(0.9368213,0.6067858,0.4629994,1).a);
				#endif
				matcap.rgb *= float(1.11);
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
				...
				blendMatcap(poiLight, poiFragData, float(0.155), float(0), float(0), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.4061945,0.2998169,0.2405164,1).rgb, float(0)), float4(0.4061945,0.2998169,0.2405164,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.4061945,0.2998169,0.2405164,1).rgb, float(0)), float4(0.4061945,0.2998169,0.2405164,1).a);
				#endif
				...
				rim *= float4(0.4061945,0.2998169,0.2405164,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.4061945,0.2998169,0.2405164,1).rgb, float(0)));
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.2)))) * float(0.328);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.2), 5)), float(0.429)) * float(0.328);
				...
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(1,0.6205249,0.4125678,1), float(0)) * lerp(1, poiLight.directColor, float(1)) * specMask * poiLight.occlusion * attenuation * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(1,0.6205249,0.4125678,1), float(0)) * lerp(1, poiLight.directColor, float(1)), saturate(specMask * poiLight.occlusion * attenuation * float(1)));
				}
				float3 vSpecMask = 0;
				#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.2)))) * float(0.328) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.2), 5)), float(0.429)) * float(0.328) * poiLight.vAttenuation * poiLight.vColor[index];
				...
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(1,0.6205249,0.4125678,1), float(0)) * vSpecMask * poiLight.occlusion * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(1,0.6205249,0.4125678,1), float(0)), saturate(vSpecMask * poiLight.occlusion * float(1)));
				}
				#endif
			}
			#endif
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				clip(poiFragData.alpha - float(0.001));
