Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/95d9ccfd85ae19c48bcf313c67ef8669"
{
	Properties
	{
 ...
 #define PROP_SET_HIGHCOLORMASK 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.552);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.552);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.552) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.25) * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.25) * blurMap.r, float(0));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.25) * blurMap.r);
					...
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.25) * blurMap.r, float(0));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.7615293,0.7615293,0.7615293,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.7615293,0.7615293,0.7615293,1).rgb;
				}
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(0.1402577,0.6481708,1,1).rgb);
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.552));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.552));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.552) * poiLight.shadowMask);
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rimWidth = lerp( - .05, 1, float(0.35));
				...
				float rim = 1 - smoothstep(min(float(0.009), rimWidth), rimWidth, viewDotNormal);
				...
				float3 finalRimColor = rimColor.rgb * lerp(1, poiFragData.baseColor, float(1));
			...
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min(float(0), float(0.336)), float(0.336), poiCam.vDotN));
				float(0.608) *= 1.7 - 0.7 * float(0.608);
				float3 enviroRimColor = 0;
				float interpolator = unity_SpecCube0_BoxMin.w;
				
				if (interpolator < 0.99999)
				{
					float4 reflectionData0 = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.608) * UNITY_SPECCUBE_LOD_STEPS);
					float3 reflectionColor0 = DecodeHDR(reflectionData0, unity_SpecCube0_HDR);
					float4 reflectionData1 = UNITY_SAMPLE_TEXCUBE_SAMPLER_LOD(unity_SpecCube1, unity_SpecCube0, poiMesh.normals[1], float(0.608) * UNITY_SPECCUBE_LOD_STEPS);
				...
				{
					float4 reflectionData = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], float(0.608) * UNITY_SPECCUBE_LOD_STEPS);
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.128)))) * float(0.364);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0.156)))) * float(0.101);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.128), 5)), float(0.439)) * float(0.364);
					specMask2 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.156), 5)), float(0.215)) * float(0.101);
				}
				#if defined(PROP_SET_HIGHCOLORMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.06731268,0.3217961,1,1), float(0)) * lerp(1, poiLight.directColor, float(0)) * specMask * poiLight.occlusion * attenuation * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.06731268,0.3217961,1,1), float(0)) * lerp(1, poiLight.directColor, float(0)), saturate(specMask * poiLight.occlusion * attenuation * float(1)));
				}
				float3 vSpecMask = 0;
				#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.128)))) * float(0.364) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0.156)))) * float(0.101) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.128), 5)), float(0.439)) * float(0.364) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, poiEdgeNonLinear(specArea, (1.0 - pow(float(0.156), 5)), float(0.215)) * float(0.101) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
				}
				vSpecMask *= specularMask;
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.06731268,0.3217961,1,1), float(0)) * vSpecMask * poiLight.occlusion * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.06731268,0.3217961,1,1), float(0)), saturate(vSpecMask * poiLight.occlusion * float(1)));
				}
				#endif
			}
			#endif
 ...
 #define PROP_SET_HIGHCOLORMASK 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.6903409,0.5590093,0.5590093,1).rgb, float(0)), float4(0.6903409,0.5590093,0.5590093,1).a);
