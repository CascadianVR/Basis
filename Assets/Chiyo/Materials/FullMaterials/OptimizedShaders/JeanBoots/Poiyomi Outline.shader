Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/c73e27f8106e1834b91fb234f2b09ee5"
{
	Properties
	{
 ...
 #define PROP_SET_HIGHCOLORMASK 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					...
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.113) * blurMap.r, float(0.327));
				}
				else
				{
					...
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.113) * blurMap.r, float(0.327));
				}
				lns = saturate(lns);
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(1,0.07184543,0.3300491,1).rgb);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				poiFragData.alpha *= lerp(1, matcap2.a, matcap2Mask * float(1));
				if (float(0))
				{
					matcap2.rgb = hueShift(matcap2.rgb, float(0) + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(0), float(1), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.49)))) * float(0.83);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0.48)))) * float(0.038);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.49), 5)), float(0.314)) * float(0.83);
					specMask2 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.48), 5)), float(0.092)) * float(0.038);
				}
				#if defined(PROP_SET_HIGHCOLORMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(1,0.8274913,0.7524076,1), float(0)) * lerp(1, poiLight.directColor, float(0)) * specMask * poiLight.occlusion * attenuation * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(1,0.8274913,0.7524076,1), float(0)) * lerp(1, poiLight.directColor, float(0)), saturate(specMask * poiLight.occlusion * attenuation * float(1)));
				}
				float3 vSpecMask = 0;
				#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.49)))) * float(0.83) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0.48)))) * float(0.038) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.49), 5)), float(0.314)) * float(0.83) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, poiEdgeNonLinear(specArea, (1.0 - pow(float(0.48), 5)), float(0.092)) * float(0.038) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
				}
				vSpecMask *= specularMask;
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(1,0.8274913,0.7524076,1), float(0)) * vSpecMask * poiLight.occlusion * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(1,0.8274913,0.7524076,1), float(0)), saturate(vSpecMask * poiLight.occlusion * float(1)));
				}
				#endif
			}
			#endif
