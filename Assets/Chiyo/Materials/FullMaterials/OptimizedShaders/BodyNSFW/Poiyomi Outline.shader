Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/39b02d9d5df94b048a663a82223fea10"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMASK 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.442);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.442);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.442) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.396), float(0.372) * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0), float(0) * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0), float(0) * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.396), float(0.372) * blurMap.r, float(0.501));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.396), float(0.372) * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0), float(0) * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0), float(0) * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.396), float(0.372) * blurMap.r, float(0.501));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(1,0.6200959,0.5225216,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(1,0.6200959,0.5225216,1).rgb;
				}
				if (float4(1,0.6172068,0.5209957,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(1,0.6172068,0.5209957,1).rgb;
					lns.y = float4(1,0.6172068,0.5209957,1).a - lns.y * float4(1,0.6172068,0.5209957,1).a;
					indirectColor = lerp(indirectColor, shadow2ndColorTex.rgb, lns.y);
				}
				if (float4(1,0.6172068,0.5209957,0).a > 0)
				{
					#if defined(PROP_SHADOW3RDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow3rdColorTex.rgb = lerp(float3(1, 1, 1), shadow3rdColorTex.rgb, shadow3rdColorTex.a) * float4(1,0.6172068,0.5209957,0).rgb;
					lns.z = float4(1,0.6172068,0.5209957,0).a - lns.z * float4(1,0.6172068,0.5209957,0).a;
					indirectColor = lerp(indirectColor, shadow3rdColorTex.rgb, lns.z);
				}
				poiLight.rampedLightMap = lns.x;
				indirectColor = lerp(indirectColor, 1, lns.w * float4(1,0.008773524,0,1).rgb);
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.442));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.442));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.442) * poiLight.shadowMask);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(poiThemeColor(poiMods, float4(0.192857,1,1,1).rgb, float(0)), float4(0.192857,1,1,1).a);
				#else
				matcap = float4(poiThemeColor(poiMods, float4(0.192857,1,1,1).rgb, float(0)), float4(0.192857,1,1,1).a);
				#endif
				matcap.rgb *= float(1);
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
				...
				blendMatcap(poiLight, poiFragData, float(0.308), float(0), float(0), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				switch(float(1))
				{
					case 0:
					{
						...
						float3 NormalBlend_MatCapUV_Base = (mul(UNITY_MATRIX_V, float4(poiCam.viewDir, 0)).rgb * float3(-1, -1, 1)) + float3(0, 0, 1);
						float3 noSknewViewNormal = NormalBlend_MatCapUV_Base * dot(NormalBlend_MatCapUV_Base, NormalBlend_MatCapUV_Detail) / NormalBlend_MatCapUV_Base.b - NormalBlend_MatCapUV_Detail;
						matcapUV = noSknewViewNormal.rg * float(0.332) + 0.5;
						break;
					}
					case 1:
					{
						float3 worldViewUp = normalize(float3(0, 1, 0) - poiCam.viewDir * dot(poiCam.viewDir, float3(0, 1, 0)));
						float3 worldViewRight = normalize(cross(poiCam.viewDir, worldViewUp));
						matcapUV = float2(dot(worldViewRight, normal1), dot(worldViewUp, normal1)) * float(0.332) + 0.5;
						break;
					}
					case 2:
					{
				...
				blendMatcap(poiLight, poiFragData, float(0.339), float(0), float(0), float(0), float(0), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				//ifex float(1)==0
				#ifdef POI_MATCAP2
				float3 normal2 = poiMesh.normals[float(1)];
				matcapUV = 0;
				switch(float(1))
				{
					case 0:
					{
						...
						float3 NormalBlend_MatCapUV_Base = (mul(UNITY_MATRIX_V, float4(poiCam.viewDir, 0)).rgb * float3(-1, -1, 1)) + float3(0, 0, 1);
						float3 noSknewViewNormal = NormalBlend_MatCapUV_Base * dot(NormalBlend_MatCapUV_Base, NormalBlend_MatCapUV_Detail) / NormalBlend_MatCapUV_Base.b - NormalBlend_MatCapUV_Detail;
						matcapUV = noSknewViewNormal.rg * float(0.383) + 0.5;
						break;
					}
					case 1:
					{
						float3 worldViewUp = normalize(float3(0, 1, 0) - poiCam.viewDir * dot(poiCam.viewDir, float3(0, 1, 0)));
						float3 worldViewRight = normalize(cross(poiCam.viewDir, worldViewUp));
						matcapUV = float2(dot(worldViewRight, normal2), dot(worldViewUp, normal2)) * float(0.383) + 0.5;
						break;
					}
					case 2:
					{
				...
				#if defined(PROP_MATCAP3) || !defined(OPTIMIZER_ENABLED)
				matcap3 = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap3, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap3)) * float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#else
				matcap3 = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#endif
				matcap3.rgb *= _Matcap3Intensity;
				#if defined(PROP_MATCAP3MASK) || !defined(OPTIMIZER_ENABLED)
				matcap3Mask = POI2D_SAMPLER_PAN(_Matcap3Mask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				matcap3Mask = 1;
				#endif
				if (float(0))
				...
				if (float(0))
				{
					matcap3Mask = lerp(matcap3Mask, 1, TPSBufferedDepth(poiMesh.localPos, poiMesh.vertexColor) * float(1));
				}
				#endif
				poiFragData.alpha *= lerp(1, matcap3.a, matcap3Mask * float(0));
				if (float(0))
				{
					matcap3.rgb = hueShift(matcap3.rgb, float(0) + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, float(0.466), float(0), float(0), float(0), float(0), matcap3, matcap3Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(0.532), float(0), float(1), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rimWidth = lerp( - .05, 1, float(0.391));
				float blendStrength = float(0.208);
				#ifdef POI_AUDIOLINK
				...
				float rim = 1 - smoothstep(min(float(0.155), rimWidth), rimWidth, viewDotNormal);
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.2)))) * float(0.124);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.2), 5)), float(0.172)) * float(0.124);
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.2)))) * float(0.124) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.2), 5)), float(0.172)) * float(0.124) * poiLight.vAttenuation * poiLight.vColor[index];
 ...
 #define PROP_EMISSIONMASK 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.673178,0.4271505,0.4271505,1).rgb, float(0)), float4(0.673178,0.4271505,0.4271505,1).a);
