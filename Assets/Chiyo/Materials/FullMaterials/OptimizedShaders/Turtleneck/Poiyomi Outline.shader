Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/db4bb557fda627e48bbb28d55e8c1923"
{
	Properties
	{
 ...
 #define _RIMSTYLE_POIYOMI 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.383);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.383);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.383) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				if (float(1))
				{
					...
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.124), float(0.318) * blurMap.g);
				...
				else
				{
					...
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.124), float(0.318) * blurMap.g);
				...
				if (float4(0.8176128,0.8176128,0.8176128,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.8176128,0.8176128,0.8176128,1).rgb;
				}
				if (float4(0.8148468,0.8148468,0.8148468,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.8148468,0.8148468,0.8148468,1).rgb;
					lns.y = float4(0.8148468,0.8148468,0.8148468,1).a - lns.y * float4(0.8148468,0.8148468,0.8148468,1).a;
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.383));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.383));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.383) * poiLight.shadowMask);
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rimWidth = lerp( - .05, 1, float(0.497));
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.391)))) * float(0.006);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.391), 5)), float(0.321)) * float(0.006);
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.391)))) * float(0.006) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.391), 5)), float(0.321)) * float(0.006) * poiLight.vAttenuation * poiLight.vColor[index];
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor = saturate(poiFragData.baseColor + float(-0.054) * hueShiftAlpha.g);
				#endif
